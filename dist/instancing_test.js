...
target.addEventListener("drop", function (event) {
    ...
    dat_reader.onload = () => {
        ...
        for (let i = 0; i < matrixArray.length; i++) {
            ...
            instanced_backbone.addAttribute(`aInstanceMatrix${i}`, new THREE.InstancedBufferAttribute(matrixArray[i], 4));
        }
        const instanceColorArray = new Uint8Array(num_nuc * 3);
        instanced_backbone.addAttribute('aInstanceColor', new THREE.InstancedBufferAttribute(instanceColorArray, 3, 1));
        ...
        instanced_backbone.addAttribute('aInstanceEmissive', instanceEmissive);
        ...
        varying vec3 vInstanceColor;
        ${shader.fragmentShader.replace('vec4 diffuseColor = vec4( diffuse, opacity );', 'vec4 diffuseColor = vec4( vInstanceColor, opacity );')}
        `;
            shader.fragmentShader = `
        varying vec3 vInstanceEmissive;
        ...
        scene.add(new THREE.Mesh(instanced_backbone, instanceMaterial));
        render();
    };
}, false);
